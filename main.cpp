...
//        int num_colors = boost::sequential_vertex_coloring(g, color);
        ...
        for (int color = min_possible_color; color < max_possible_color; color++) {
            ...
            out << "max_color " << color << endl;
            out << "num_colors (nat, new, lfo, sat): " << num_colors_natural << " " << num_colors_newIdea << " "
                << num_colors_lfo << " " << num_colors_sat << endl;
            out << "num_misses (nat, new, lfo, sat): " << all_misses_natural << " " << all_misses_newIdea << " "
                << all_misses_lfo << " " << all_misses_sat << endl << endl;
        }
    }
    out.flush();
    out.close();
...
int compute_misses(int num_colors, const std::vector<int> &color_vec, boost::numeric::ublas::matrix<int> &m) {
    int all_sum = 0;
    std::vector<boost::numeric::ublas::vector<int>> misses(num_colors + 1);
